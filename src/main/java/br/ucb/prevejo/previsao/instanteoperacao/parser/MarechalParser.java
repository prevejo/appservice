	...
	public Collection<InstanteOperacao> parse(String str) throws ParseException {
		...
		try {
			...
			for (Object o : array) {
				...
				Point point = Geo.makePoint((Double) arrayPosicao.get(2), (Double)arrayPosicao.get(3));
				...
				Velocidade velocidade = !StringUtil.isEmpty(arrayPosicao.get(7))
						? Velocidade.kilometrosPorHora(new BigDecimal(arrayPosicao.get(7).toString().replace(",", "."))) : null;
